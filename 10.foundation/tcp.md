...
**Transmission Control Protocol** (**TCP**) — это L4-протокол, обеспечивающий надёжное виртуальное соединение между процессами. Применяет **TCP-сегменты**.
...
sequenceDiagram
  ...
  A->>B: ACK
  Note over A, B: Подтверждение соединения
```
...
sequenceDiagram
  ...
  Note over A, B: Запрос подтверждения
  A->>B: ACK
  Note over A, B: Подтверждение завершения
```
